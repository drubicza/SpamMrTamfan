...
$file_lisensi = dirname(__FILE__).'/.BomSMS';
$hwid = str_replace(":", "",`ifconfig | grep -E "Ethernet" | awk '{print $5}'`);
$hwid = explode("\n", $hwid);
$hwid = array_filter(array_diff($hwid, preg_grep("/0{12,}/", $hwid)));

if (!empty($hwid[0])) {
    $hwid = strtoupper($hwid[0]);
} else {
    $hwid = "1C46B920A5ED";
}
...
if (!file_exists($file_lisensi)) {
    ...
    if (isset($lisensi)) {
        $fp = fopen($file_lisensi, "w") or die(" \033[1;31m[!] Terjadi kesalahan perijinan storage!!!\033[1;33m\n");
        fwrite($fp, $lisensi);
        fclose($fp);
    } else {
      exit;
...
if ($chk_lisensi == "valid") {
    ...
    unlink($file_lisensi);
    die("\033[1;31m[!] Lisensi anda salah, silahkan hubungi pembuat!!!\033[1;33m\n");
}
